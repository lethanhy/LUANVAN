<template>
    <div class="card--room">
      ...
      <div class="room">
        <div v-for="room in rooms" :key="room._id">
          ...
          <div v-else class="inactive-room">
            <p>Phòng {{ room.roomNumber }}</p>
            Tạm ngưng hoạt động
          </div>
        </div>
      </div>
  ...
  .inactive-room {
    ...
    width: 100%; /* Chiếm hết chiều rộng của khối cha */
    height: auto; /* Đặt chiều cao tự động */
    background-color: #f8d7da; /* Màu đỏ nhạt */
    border: 1px dashed #dc3545; /* Viền đỏ */
    padding: 1rem;
    border-radius: 10px;
    text-align: center;
    color: #dc3545; /* Chữ màu đỏ */
    font-weight: bold;
    box-shadow: none; /* Xóa bóng nếu bị áp dụng từ nơi khác */
